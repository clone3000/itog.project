...
//        System.out.println(categoryRepository.findAll().size());
    ...
    public String changeStatus(@PathVariable("id") int id, @RequestParam("status") String status){
        orderRepository.updateStatus(id, Status.valueOf(Integer.parseInt(status)));
        return "redirect:/admin/orders";
    }

    @PostMapping("/orders/search")
    public String findOrder(@RequestParam("orderNum") String orderNum, Model model){
        model.addAttribute(
            "orders",
            orderNum.isEmpty()
                ?orderRepository.findAll()
                :orderRepository.findByLastOrderSymbols(orderNum)
        );
        return "admin/orders";
    }
}
